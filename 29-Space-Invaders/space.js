...
function update() {
    ...
    for (let i = 0; i < alienArray.length; i++) {
        ...
        if (alien.alive) {
            ...
            // تعديل موضع رسالة انتهاء اللعبة
            if (alien.y >= ship.y) {
                gameOver = true;
                context.fillStyle = "white";
                context.font = "32px Tajawal";
                let gameOverText = "انتهت اللعبة";
                let textWidth = context.measureText(gameOverText).width;
                context.fillText(gameOverText, boardWidth/2 + textWidth/2, boardHeight/2);
            }
        }
    }
    ...
    let scoreText = "النقاط: " + score;
    let scoreWidth = context.measureText(scoreText).width;
    context.fillText(scoreText, (boardWidth - scoreWidth)/2, 20);
}
