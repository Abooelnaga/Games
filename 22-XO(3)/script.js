...
function restartGame(ask) {
    if (moves > 0) {
        var response = confirm("هل أنت متأكد أنك تريد البدء من جديد؟");
        if (response === false) {
            return;
        }
    }
...
function endGame(who) {
    if (who == player) {
        announceWinner("مبروك! لقد فزت!");
    } else if (who == computer) {
        announceWinner("الحاسوب هو الفائز!");
    } else {
        announceWinner("تعادل!");
    }
    gameOver = true;
    whoseTurn = 0;
    moves = 0;
