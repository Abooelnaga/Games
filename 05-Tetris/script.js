(function () {
  ...
  var tetris = {
    ...
    boardDiv: null,
    canvas: null,
    pSize: 20,         // حجم المكعب 20px
    canvasHeight: 600, // نفس ارتفاع الـ CSS
    canvasWidth: 300,  // نفس عرض الـ CSS
    boardHeight: 0,
    boardWidth: 0,
    spawnX: 7,         // نقطة البداية في المنتصف (15/2)
    spawnY: 1,
    shapes: [
      [
        [-1, 1],
    ...
    initBoard: function () {
      this.boardHeight = this.canvasHeight / this.pSize; // 600 / 20 = 30
      this.boardWidth = this.canvasWidth / this.pSize;   // 300 / 20 = 15
    ...
    removeCur: function () {
      var me = this;
      this.curSqs.forEach(function (sq) {
        me.canvas.removeChild(sq);
      });
      this.curSqs = [];
    },
    ...
    gameOver: function () {
      this.clearTimers();
      isStart = false;
      this.canvas.innerHTML = '<h1 style="color: white; text-align: center; padding-top: 250px;">انتهت اللعبة</h1>';
    ...
    rotate: function () {
      if (this.curShapeIndex !== 6) { // square
        var temp = [];
        this.curShape.forEach(function (block) {
          temp.push([block[1] * -1, block[0]]);
        });
    ...
    removeBlock: function (x, y) {
      var me = this;
      this.markBoardAt(x, y, 0);
      this.sqs.forEach(function (sq, i) {
        if (me.getPos(sq)[0] === x && me.getPos(sq)[1] === y) {
          me.canvas.removeChild(sq);
          me.sqs.splice(i, 1);
        }
      });
